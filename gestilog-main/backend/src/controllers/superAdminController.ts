...
export const getMagasins = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM magasins');
...
export const updateMagasin = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await logActivity(req, 'modification_magasin', 'magasin', parseInt(id), updates);
  ...
  try {
    ...
    const expirations = await pool.query(
      `SELECT id, nom_magasin, email, date_expiration_abonnement
       FROM magasins
       WHERE date_expiration_abonnement BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'
       AND statut = 'actif'
