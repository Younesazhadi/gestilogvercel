        ...
        <table className="min-w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Crédit autorisé</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {clients.map((client) => (
              <tr key={client.id} className={`hover:bg-gray-50 ${client.solde > 0 ? 'bg-red-50' : ''}`}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex flex-col">
                    <span className={`font-bold ${client.solde > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      {Number(client.solde).toFixed(2)} MAD
                    </span>
                    {client.solde > 0 && (
                      <span className="text-xs text-red-500 mt-1">⚠️ Crédit en cours</span>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-gray-600">
                    {Number(client.credit_autorise).toFixed(2)} MAD
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex space-x-3">
                    {client.solde && client.solde > 0 && (
                      <button
                        onClick={() => navigate(`/admin/pos?client_id=${client.id}&payer_credit=true`)}
