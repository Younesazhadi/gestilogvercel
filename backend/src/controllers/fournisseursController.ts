...
export const getFournisseurs = async (req: AuthRequest, res: Response) => {
  try {
    ...
    let countQuery = `SELECT COUNT(*) as total FROM fournisseurs WHERE magasin_id = $1`;
    const countParams: any[] = tenantFilter.params.slice();

    if (search) {
      countQuery += ` AND (nom ILIKE $${countParams.length + 1} OR contact_nom ILIKE $${countParams.length + 1})`;
